...
/** The SVG of an already-assembled figure (the render handle's path). */
export function svgFromAssembled(asm: Assembled, size: ViewSize): string {
  const { geom } = asm.realized.group;
  const paths = buildPathList(asm.scene, { geom, model: asm.realized.model, camera: asm.camera, size });
  return toSvg(mergeFieldPaths(paths), asm.camera, size);
}

/** The PNG of an already-assembled figure: field (if any) under the vector overlay, at k×. */
  ...
  const layers: RasterLayer[] =
    asm.field !== null && asm.overlay !== null
      ? [
          ...
          sceneLayer(asm.overlay, geom, asm.realized.model),
        ]
      : [sceneLayer(asm.scene, geom, asm.realized.model)];
  return renderPng(layers, asm.camera, size, k, background);
}

/** Raw document → SVG string. Pure — no DOM anywhere. */
export function figureToSvg(figure: unknown, opts?: ExportOptions): ExportResult<string> {
  const r = assembleForExport(figure, opts);
  if (!r.ok) return r;
  return { ok: true, value: svgFromAssembled(r.asm, r.size) };
}
...
export async function figureToPng(
  ...
  if (!r.ok) return r;
  try {
    return { ok: true, value: await pngFromAssembled(r.asm, r.size, k, opts?.background) };
  } catch (e) {
    return { ok: false, problems: [{ path: '', problem: e instanceof Error ? e.message : String(e) }] };
  }
}
