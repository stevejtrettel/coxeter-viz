...
| file | exports |
|---|---|
| `realize.ts` | `defaultModel(kind)` (Poincaré/Cartesian/Stereographic by geometry) · `polygonSpec(orders, geometry?)` (geometry defaults to `classifyPolygon`) · `realizeSpec(spec, opts?) → RealizedGroup` (P3: the spec-shaped entry — the inference layer / `app/` path; composes `solvePolygon`+`groupFromPolygon`+`defaultModel`) · `realizePolygon(orders, opts?)` (= `realizeSpec(polygonSpec(orders))`) |
